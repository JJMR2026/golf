...
window.handleAuth = async function(type) {
    ...
    if (!supabaseClient) return alert("Database connection failed.");
    ...
    try { 
        ...
        if (errEl) { errEl.style.color = "#ef4444"; errEl.innerText = e.message; } else { alert(e.message); }
    } finally { 
...
window.handleForgotPassword = async function() {
    const cleanInput = document.getElementById('auth-nickname').value.trim().replace(/\s+/g, ''); 
    const email = cleanInput.includes('@') ? cleanInput : cleanInput.toLowerCase() + "@golf.local";
    if (!email || !email.includes('@') || email.includes('@golf.local')) return alert("Requires a valid verification email.");
...
window.checkHarvesterStatus = async function() {
    ...
    if (!supabaseClient) return document.getElementById('insight-detail-content').innerHTML = "❌ Database Offline.";
...
window.updatePracticeTable = function() {
    ...
    if(practiceSessionData.length === 0) { 
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding: 15px;">No shots logged.</td></tr>'; return; 
    }
    
    let grouped = {};
    practiceSessionData.forEach(s => { 
        if(!grouped[s.club]) grouped[s.club] = { tot: 0, cnt: 0, shapes: {} }; 
        grouped[s.club].tot += s.dist; grouped[s.club].cnt++; 
...
window.savePracticeSession = function() {
    if(practiceSessionData.length === 0) return alert("Session empty.");
    alert("Practice telemetry array synchronized to terminal memory stacks."); 
    practiceSessionData = []; window.updatePracticeTable();
};
...
window.fetchWeatherForCourse = function(courseName) {
    ...
    .then(res => res.json())
    .then(data => {
        if (data && data.length > 0) window.fetchWeatherByCoords(data[0].lat, data[0].lon, display, courseName);
        else {
    ...
    searchInputEl.addEventListener('input', e => {
        ...
        searchTimeout = setTimeout(async () => { 
            ...
            try { 
                ...
                if(error) throw error;
                const uniqueCourses = [];
                data.forEach(item => { let c = item.course_name.trim(); if (!uniqueCourses.includes(c)) uniqueCourses.push(c); });
...
window.populateClubDropdowns = function() {
    ...
    if (driveSelect && appSelect) {
        ...
        let html = '<option value="">-- Select Club --</option>';
...
window.toggleDropType = function(type) {
    ...
    if (index > -1) adv.splice(index, 1); else adv.push(type);
    ...
    ['WATER', 'OB', 'LOST', 'UNPLAYABLE'].forEach(id => { 
        ...
        if(btn) { if (adv.indexOf(id) > -1) btn.classList.add('active'); else btn.classList.remove('active'); }
    }); 
    window.saveLocalState();
};
...
window.setPlayToggle = function(type, status) {
    ...
    if (status === 'hit') {
        if (current === 'hit') nextStatus = (type === 'fir') ? 'drv grn' : 'under';
        else if (current === 'drv grn' || current === 'under') nextStatus = "";
    } else if (status === 'miss') {
        if (current === 'miss') nextStatus = "";
    }
    
    roundData[currentPlayHole][type] = nextStatus; 
    if (nextStatus !== 'miss') roundData[currentPlayHole][type + 'Adv'] = [];
    ...
    if(gridCell) { 
        ...
        if (nextStatus === 'hit' || nextStatus === 'drv grn' || nextStatus === 'under') gridCell.classList.add('hit');
        else gridCell.classList.remove('hit');
    }
    window.updatePlayModeUI(); 
    window.saveLocalState();
};
...
window.toggleAdv = function(type, val) {
    ...
    window.updatePlayModeUI(); window.saveLocalState();
};
...
window.cycleSand = function() {
    ...
    if(gridCell) { 
        ...
        if (next === "1") gridCell.classList.add('hit'); else gridCell.classList.remove('hit');
    }
    window.updatePlayModeUI(); window.saveLocalState();
};
...
window.syncPlayToState = function(field, val) {
    ...
    window.saveLocalState(); window.updatePlayModeUI();
};
...
window.updatePlayModeUI = function() {
    ...
    for(let i=currentHoleOffset; i<endIndex; i++) {
        ...
        if(!isNaN(s)) { totScore += s; currPar += (!isNaN(p) ? p : 4); }
    }
    let rel = totScore - currPar;
    let relStr = rel > 0 ? `+${rel}` : (rel === 0 ? 'E' : `${rel}`);
    
    let statsHeader = document.getElementById('play-stats-header');
    const container = document.getElementById('play-mode-container');
    if (!statsHeader) {
        statsHeader = document.createElement('div');
        statsHeader.id = 'play-stats-header';
        statsHeader.style = "text-align:center; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-bottom: 10px; font-weight: bold; color: var(--text-muted); font-size: 14px;";
        container.prepend(statsHeader);
    }
    statsHeader.innerHTML = `COURSE PAR: <span style="color:var(--text-main)">${totPar}</span> &nbsp;|&nbsp; CURRENT SCORE: <span style="color:var(--accent-green)">${totScore} (${relStr})</span>`;

    let navHtml = `
        <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
            <button class="nav-btn" onclick="window.changePlayHole(-1)" style="flex:1; margin-right:5px; padding:10px;">◀ Prev Hole</button>
            <button class="nav-btn" onclick="window.changePlayHole(1)" style="flex:1; margin-left:5px; padding:10px;">Next Hole ▶</button>
        </div>
    `;
    let navTop = document.getElementById('play-nav-top');
    let navBot = document.getElementById('play-nav-bot');
    if(!navTop) { navTop = document.createElement('div'); navTop.id = 'play-nav-top'; navTop.innerHTML = navHtml; statsHeader.after(navTop); }
    if(!navBot) { navBot = document.createElement('div'); navBot.id = 'play-nav-bot'; navBot.innerHTML = navHtml; container.appendChild(navBot); }
    ...
    if (state.score === "") {
        ...
        if (par == 4 || par == 5) { 
            if (state.driveClub === "") { 
                let dClubs = [];
                masterAnalyticsData.forEach(r => { (r.hole_scores||[]).forEach(h => { if (h.par == par && h.drive_club && bag.includes(h.drive_club)) dClubs.push(h.drive_club); }); });
        ...
        if (par == 3) { 
            if (state.appClub === "") { 
                let aClubs = [];
                masterAnalyticsData.forEach(r => { (r.hole_scores||[]).forEach(h => { if (h.par == 3 && h.approach_club && bag.includes(h.approach_club)) aClubs.push(h.approach_club); }); });
            ...
            state.fir = "hit"; 
            const fCell = document.getElementById(`grid-fir-${currentPlayHole}`); if(fCell) fCell.innerText = "HIT"; 
        }
    }
    ...
    ['fir', 'gir'].forEach(type => {
        ...
        if(hb) {
            ...
            if (stateVal === 'hit' || stateVal === 'drv grn' || stateVal === 'under') {
                hb.classList.add('active'); 
                if (stateVal === 'drv grn') hb.innerText = "DRV GRN";
                else if (stateVal === 'under') hb.innerText = "UNDER REG";
                else hb.innerText = "HIT";
            } else { hb.classList.remove('active'); hb.innerText = "HIT"; }
        }
        if(mb) { if (state[type] === 'miss') mb.classList.add('active'); else mb.classList.remove('active'); }
        if(subMenu) { 
            ...
            document.querySelectorAll(`#${type}-sub-menu .sub-miss`).forEach(btn => { 
                ...
                if (advArr.indexOf(val) > -1) btn.classList.add('active'); else btn.classList.remove('active');
            }); 
        }
    });
    ...
    if (currentUser && currentUser.email === 'jordanrohel@yahoo.ca') {
        if(!tmplBtn) {
            tmplBtn = document.createElement('button'); tmplBtn.id = 'admin-save-template-btn'; tmplBtn.className = 'primary';
            tmplBtn.style.width = '100%'; tmplBtn.style.marginTop = '15px'; tmplBtn.style.marginBottom = '15px'; tmplBtn.style.background = '#f59e0b'; tmplBtn.style.color = '#000';
            tmplBtn.innerText = '💾 ADMIN: PUSH OVERRIDES TO DB'; tmplBtn.onclick = window.saveCourseTemplate;
            navBot.after(tmplBtn);
        }
        tmplBtn.style.display = 'block';
    } else { if (tmplBtn) tmplBtn.style.display = 'none'; }
...
window.fetchCourseDetails = async function() {
    ...
    const originalText = fetchBtn.innerText; fetchBtn.innerText = "⏳..."; fetchBtn.disabled = true; 
    document.getElementById('api-status').innerText = "Loading...";

    if (!supabaseClient) { document.getElementById('api-status').innerText = "⚠️ Offline mode. Type course and hit Start Round."; fetchBtn.innerText = originalText; fetchBtn.disabled = false; return; }
    ...
    try {
        ...
        if (teeData) {
            ...
            if (matchedCourse) {
                ...
                if (typeof parsedPars === 'string') { try { parsedPars = JSON.parse(parsedPars.replace(/{/g, '[').replace(/}/g, ']')); } catch(e){} }
                ...
                window.populateTeeDropdown(); document.getElementById('api-status').innerText = ""; 
                window.buildGrid(); window.updatePlayModeUI(); window.saveLocalState(); return;
            }
        }
    } catch(e) {} finally { fetchBtn.innerText = originalText; fetchBtn.disabled = false; }
    ...
    currentCoursePars = Array(18).fill(""); currentYardages = Array(18).fill(""); availableTees = []; 
    window.populateTeeDropdown(); window.buildGrid(); window.updatePlayModeUI(); window.saveLocalState();
};
...
window.handleTeeChange = function() {
    ...
    window.buildGrid(); window.updatePlayModeUI(); window.saveLocalState();
};
...
window.setHoleCount = function(count) { 
    ...
    let toggleBox = document.getElementById('front-back-toggle');
    if (count === 9) { 
        toggleBox.style.display = 'inline-flex'; currentHoleOffset = 0; 
    ...
    currentPlayHole = currentHoleOffset; window.buildGrid(); window.updatePlayModeUI(); window.saveLocalState(); 
};
...
window.buildGrid = function() {
    const grid = document.getElementById('scorecard-grid'); if(!grid) return; grid.innerHTML = ''; 
    ...
    rows.forEach(row => {
        ...
        for (let i = currentHoleOffset; i < endIndex; i++) {
            ...
            let pVal = currentCoursePars[i]; pVal = (pVal !== undefined && pVal !== null) ? pVal : '';
            let yVal = currentYardages[i]; yVal = (yVal !== undefined && yVal !== null) ? yVal : '';
            
            if (row.type === 'header') { 
                cell.className = 'cell hole-header'; cell.innerHTML = `<button type="button" onclick="window.jumpToPlayMode(${i})" style="width:100%;height:100%;background:transparent;border:none;color:var(--text-muted);font-weight:bold;cursor:pointer;padding:0;">${i + 1}</button>`; 
            } else if (row.type === 'par') { 
                cell.className = 'cell'; cell.innerHTML = `<input type="number" id="par-input-${i}" inputmode="numeric" min="3" max="6" value="${pVal}" onchange="window.updatePar(${i}, this.value)">`; 
            } else if (row.type === 'yardage') { 
                cell.className = 'cell'; cell.innerHTML = `<input type="number" inputmode="numeric" value="${yVal}" onchange="currentYardages[${i}] = this.value; window.updatePlayModeUI(); window.saveLocalState();">`; 
                ...
                cell.className = 'cell'; let cVal = parseInt(roundData[i].drops) || 0; 
                ...
                cell.className = 'cell'; let cVal = roundData[i][row.type]; let btnText = "-";
                if (row.type === 'sandSave') { btnText = cVal === "" ? "-" : cVal; } else {
                    if (cVal === 'hit') btnText = 'HIT'; else if (cVal === 'miss') btnText = 'MISS'; else if (cVal === 'drv grn') btnText = 'DRV GRN'; else if (cVal === 'under') btnText = 'UNDER';
                }
                let isHit = false; if (cVal === 'hit' || cVal === '1' || cVal === 'drv grn' || cVal === 'under') isHit = true;
...
window.forceSubmitRound = async function() {
    ...
    let teeName = null; let rType = document.getElementById('round-type-select').value;
    
    if (teeVal === 'new') { 
        teeName = document.getElementById('setup-tee').value.trim(); 
        if (teeName && supabaseClient) { try { await supabaseClient.from('course_tees').insert([{ course_name: courseName, tee_name: teeName, pars: currentCoursePars, yardages: currentYardages }]); } catch(e) {} } 
    } else if (selectedTee) { teeName = selectedTee.tee_name.trim(); }

    let finalTeeName = teeName + (rType !== 'Regular' ? ' [' + rType + ']' : '');
    let totalScore = 0; let totalPutts = 0; const holesPayload = []; let missingCheck = []; 
    let endIndex = currentHoleOffset + currentHoleCount;
    ...
    for (let i = currentHoleOffset; i < endIndex; i++) {
        ...
        if (!isNaN(s)) {
            ...
            holesPayload.push({ 
                ...
                score: s, putts: validPutts, 
                fir: roundData[i].fir || null, fir_adv: (roundData[i].firAdv || []).join(','), 
...
window.openHistoryModal = async function(id, name, date, score, holesPlayed, temp, wind) {
    ...
    activeModalRoundId = id; document.getElementById('history-modal').style.display = 'flex'; 
...
window.buildModalGrid = function(holesCount, startOffset) {
    const grid = document.getElementById('modal-scorecard-grid'); grid.innerHTML = ''; grid.style.gridTemplateColumns = `70px repeat(${holesCount}, 48px)`; 
    ...
    rows.forEach(r => {
        ...
        for(let i=0; i<holesCount; i++) {
            ...
            if(r.type === 'header') { c.className = 'cell hole-header'; c.innerText = i+1+startOffset; } 
                ...
                let v = modalRoundData[i][r.type]; let t = r.type==='sandSave'?(v==='yes'?'SAVE':(v==='no'?'MISS':(v==='stuck'?'STUCK':'-'))):(v==='hit'?'HIT':(v==='miss'?'MISS':'-')); 
                let isHit = false; if (v==='hit'||v==='yes'||v==='1') isHit = true;
...
window.toggleModalDrops = function(btn, i) { 
    let cVal = parseInt(modalRoundData[i].drops) || 0; let newVal = cVal >= 4 ? 0 : cVal + 1; 
    modalRoundData[i].drops = newVal; btn.innerText = newVal === 0 ? "-" : newVal; btn.style.color = newVal > 0 ? "#ef4444" : "var(--text-muted)"; 
};

window.toggleModalHit = function(b, i, t) { 
    let ns = "";
    if (t === 'sandSave') { let cur = b.innerText; ns = cur === "-" ? "1" : (cur === "1" ? "2" : (cur === "2" ? "3+" : "")); b.innerText = ns === "" ? "-" : ns; } 
    else { ns = b.innerText === "MISS" ? "hit" : "miss"; b.innerText = ns === "" ? "-" : ns.toUpperCase(); }
    if(ns==='hit'||ns==='1') b.classList.add('hit'); else b.classList.remove('hit'); 
...
window.saveModalChanges = async function(id, holesCount) {
    ...
    const saveBtn = document.getElementById('modal-save-btn'); const originalText = saveBtn.innerText; saveBtn.innerText = "⏳ Saving..."; saveBtn.disabled = true;
    try {
        if (!supabaseClient) throw new Error("Database offline.");
        for(let i=0; i<holesCount; i++) { 
            const hd=modalRoundData[i]; const s=parseInt(hd.score); 
            if(!isNaN(s))tScore+=s; if(!isNaN(parseInt(hd.putts)))tPutts+=parseInt(hd.putts);
            if(hd.id) { await supabaseClient.from('hole_scores').update({ par:parseInt(modalCoursePars[i])||null, score:s||null, putts:hd.putts!==""?parseInt(hd.putts):null, fir:hd.fir||null, gir:hd.gir||null, drive_distance:parseInt(hd.drive)||null, drops:parseInt(hd.drops)||0, sand_save:hd.sandSave||null }).eq('id',hd.id); }
        }
...
window.calculateHandicap = function(allRounds) {
    ...
    let diffs = validRounds.map(r => {
        if (r.course_rating && r.slope_rating) {
            return ((r.total_score - r.course_rating) * 113 / r.slope_rating);
        } else {
            let parSum = r.hole_scores ? r.hole_scores.reduce((sum, h) => sum + (h.par || 0), 0) : 72;
            return (r.total_score - parSum) * 0.96;
        }
    }).sort((a,b) => a-b);
...
window.updateTrophyRoom = function(fRounds) {
    ...
    fRounds.forEach(r => {
        ...
        if (holesPlayed >= 18) {
            ...
            if (r.total_score > 0) { 
                if (r.total_score < minScore) { minScore = r.total_score; lowScores = [{c: (r.course_name||"").trim(), d: dStr, p: coursePar}]; } 
            ...
            if (hp >= 18 && truePutts > 0) { 
                if (truePutts < minPutts) { minPutts = truePutts; lowPuttsList = [{c: (r.course_name||"").trim(), d: dStr}]; } 
                else if (truePutts === minPutts) { lowPuttsList.push({c: (r.course_name||"").trim(), d: dStr}); } 
            }
            
            let firs = r.hole_scores.filter(h => h.fir === 'hit' || h.fir === 'drv grn').length;
            if (firs > maxFir) { maxFir = firs; mostFirsList = [{c: (r.course_name||"").trim(), d: dStr}]; } 
            else if (firs === maxFir && firs > 0) { mostFirsList.push({c: (r.course_name||"").trim(), d: dStr}); }
        }
        
        if (r.hole_scores) {
            r.hole_scores.forEach(h => {
                if (h.drive_distance > 0 && (!h.drive_exception || h.drive_exception === "")) {
                    if (h.drive_distance > maxDrive) { maxDrive = h.drive_distance; longDrives = [{c: (r.course_name||"").trim(), d: dStr}]; } 
                    else if (h.drive_distance === maxDrive) { longDrives.push({c: (r.course_name||"").trim(), d: dStr}); }
                }
            });
        }
    });
    ...
    tBox.innerHTML = `
        ...
        <div style="${tStyle}"><div style="font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Low Round</div><div style="font-size: 24px; color: var(--accent-green); font-weight: bold;">${displayScore}</div><div style="font-size: 11px; color: var(--text-muted); margin-top: 5px;">${lowScores.length ? `<strong>${lowScores[0].c}</strong><br><span style="opacity:0.6">${lowScores[0].d}</span>` : '--'}</div></div>
        <div style="${tStyle}"><div style="font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Long Drive</div><div style="font-size: 24px; color: var(--accent-green); font-weight: bold;">${maxDrive === 0 ? '--' : maxDrive + 'y'}</div><div style="font-size: 11px; color: var(--text-muted); margin-top: 5px;">${longDrives.length ? `<strong>${longDrives[0].c}</strong><br><span style="opacity:0.6">${longDrives[0].d}</span>` : '--'}</div></div>
        <div style="${tStyle}"><div style="font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Fewest Putts</div><div style="font-size: 24px; color: var(--accent-green); font-weight: bold;">${minPutts === 999 ? '--' : minPutts}</div><div style="font-size: 11px; color: var(--text-muted); margin-top: 5px;">${lowPuttsList.length ? `<strong>${lowPuttsList[0].c}</strong><br><span style="opacity:0.6">${lowPuttsList[0].d}</span>` : '--'}</div></div>
        <div style="${tStyle}"><div style="font-size: 11px; color: var(--text-muted); text-transform: uppercase; font-weight: bold; margin-bottom: 5px;">Most FIRs</div><div style="font-size: 24px; color: var(--accent-green); font-weight: bold;">${maxFir}</div><div style="font-size: 11px; color: var(--text-muted); margin-top: 5px;">${mostFirsList.length ? `<strong>${mostFirsList[0].c}</strong><br><span style="opacity:0.6">${mostFirsList[0].d}</span>` : '--'}</div></div>
    `;
};
...
window.generateInsights = function(fRounds) {
    ...
    const agg = (arr) => {
        ...
        arr.forEach(r => {
            if(r.weather_temp && r.weather_wind && r.total_score > 0) {
                let t = parseInt(String(r.weather_temp).replace('°C', '')); let w = parseInt(String(r.weather_wind).replace('km/h', '')); let par = r.hole_scores ? r.hole_scores.reduce((sum, h) => sum + (h.par || 0), 0) : 72;
                if(!isNaN(t) && !isNaN(w)) { stats.temp.push(t); stats.wind.push(w); stats.sc.push(r.total_score - par); }
            }
    ...
    if (glob.hp > 0 && rec.hp > 0 && fRounds.length > 3) { 
        ...
        let diff = rAvg - gAvg;
        if (diff > 0.2) insights.push(`<div class="insight-btn"><span style="font-size:18px;">🔴</span><div><b>Putting Slump:</b> You are averaging ${rAvg.toFixed(1)} putts/hole over your last 5 rounds, worse than your overall ${gAvg.toFixed(1)} avg.</div></div>`); 
...
window.updateAnalytics = function() {
    try {
        let actCrs = []; document.querySelectorAll('.course-cb:checked').forEach(cb => { actCrs.push(cb.value.replace(/&quot;/g, '"').replace(/&#39;/g, "'")); });
        let actYrs = []; document.querySelectorAll('.year-cb:checked').forEach(cb => { actYrs.push(cb.value); });
        let actMonths = []; document.querySelectorAll('.month-cb:checked').forEach(cb => { actMonths.push(cb.value); });
        let actPars = []; document.querySelectorAll('.par-cb:checked').forEach(cb => { actPars.push(cb.value); });
        let actHoles = []; document.querySelectorAll('.hole-cb:checked').forEach(cb => { actHoles.push(cb.value); });

        if(actCrs.length === 0) { masterAnalyticsData.forEach(r => { let c = (r.course_name || "").trim(); if (!actCrs.includes(c)) actCrs.push(c); }); }
        if(actYrs.length === 0) { masterAnalyticsData.forEach(r => { let y = r.date_played ? new Date(r.date_played).getUTCFullYear().toString() : new Date().getUTCFullYear().toString(); if (!actYrs.includes(y)) actYrs.push(y); }); }
        ...
        let tfEl = document.getElementById('filter-timeframe'); const timeframe = tfEl ? tfEl.value : 'season'; 
        let hfEl = document.getElementById('filter-hole-count'); const holeFilter = hfEl ? hfEl.value : 'all'; 
        ...
        if(fRounds.length === 0) { 
            ...
            let aiBox = document.getElementById('ai-insights-box'); if (aiBox) aiBox.innerHTML = "Not enough data.";
            let hcpBox = document.getElementById('hcp-display'); if (hcpBox) hcpBox.innerText = "--.-";
            let trBox = document.getElementById('trophy-room-box'); if (trBox) trBox.innerHTML = "";
            return; 
        }
        ...
        if(typeof window.generateInsights === 'function') { let aiBox = document.getElementById('ai-insights-box'); if (aiBox) aiBox.innerHTML = window.generateInsights(fRounds); }
        if(typeof window.updateTrophyRoom === 'function') { window.updateTrophyRoom(fRounds); }
        ...
        fRounds.forEach(r => { 
            ...
            th.forEach(h => { 
                ...
                if (h.par) { const d = h.score - h.par; if(d===-1) s.brd++; else if(d===0) s.par++; else if(d===1) s.bog++; }
                if(h.putts !== null && h.putts > 0) { s.putts+=h.putts; s.pHP++; } 
                if(h.drops) s.drp+=h.drops; 
                if(h.fir=='hit'||h.fir=='miss'||h.fir=='drv grn') { s.fT++; if(h.fir=='hit'||h.fir=='drv grn') s.fH++; } 
                if(h.gir=='hit'||h.gir=='miss'||h.gir=='under') { s.gT++; if(h.gir=='hit'||h.gir=='under') s.gH++; } 
...
document.addEventListener('click', function(e) { 
    if (!e.target.closest('.multi-select-container') && !e.target.closest('summary')) { document.querySelectorAll('.multi-select-dropdown').forEach(d => d.style.display = 'none'); } 
});

window.toggleGroupToggles = function(mainCb, childClass, btnTextId, defaultText) { 
    document.querySelectorAll(childClass).forEach(b => b.checked = mainCb.checked); window.updateAnalytics(); 
};
...
window.renderCharts = function(filteredRounds, actHoles, actPars) {
    ...
    let metricEl = document.getElementById('primary-chart-metric'); const activeOverlay = metricEl ? metricEl.value : 'none'; 
    let baseScores = [];
    
    chartData.forEach(r => { 
        let targetHoles = r.hole_scores || []; 
        let filteredHoles = [];
        targetHoles.forEach(h => { if (actHoles.includes(h.hole_number.toString()) && actPars.includes(h.par.toString())) { filteredHoles.push(h); } });
        targetHoles = filteredHoles;
        let rs = 0; 
        if (targetHoles.length === 0 && (!r.hole_scores || r.hole_scores.length === 0)) { rs = r.total_score; } else { targetHoles.forEach(h => { rs += (h.score || 0); }); }
        baseScores.push(rs); 
    });
    ...
    if (activeOverlay === 'hcp') {
        ...
        chartData.forEach(r => {
            ...
            th.forEach(h => {
                ...
                if(h.putts !== null && h.putts > 0) { p+=h.putts; tpA_T++; if(h.putts < 3) tpA_H++; }
    ...
    try { if(pC && typeof Chart !== 'undefined') { scorePieChart = new Chart(pC.getContext('2d'), { type: 'doughnut', data: { labels: ['Birdie or Better', 'Par', 'Bogey', 'Double+'], datasets: [{ data: [tpBrd, tpPar, tpBog, tpDbl], backgroundColor: ['#38bdf8', '#10b981', '#f59e0b', '#ef4444'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: {color: '#9ca3af', font: {size: 10}} } } } }); } } catch(e){}

    let dW=0, dOB=0, dL=0, dU=0; 
    chartData.forEach(r => { (r.hole_scores||[]).forEach(h => { let dA = h.drops_adv || ""; if(h.drops && h.drops > 0) { if(dA.includes('WATER')) dW++; if(dA.includes('OB')) dOB++; if(dA.includes('LOST')) dL++; if(dA.includes('UNPLAYABLE')) dU++; } }); });
    try { if(pCtx && typeof Chart !== 'undefined') { penaltyPieChartObj = new Chart(pCtx.getContext('2d'), { type: 'doughnut', data: { labels: ['Water', 'OB', 'Lost', 'Unplayable'], datasets: [{ data: [dW, dOB, dL, dU], backgroundColor: ['#38bdf8', '#ef4444', '#f59e0b', '#8b5cf6'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: {color: '#9ca3af', font: {size: 10}} } } } }); } } catch(e){}

    let accLabels = [], firData = [], girData = [];
    chartData.forEach(r => {
        let fH=0, fT=0, gH=0, gT=0; 
        (r.hole_scores || []).forEach(h => { if(h.fir === 'hit' || h.fir === 'drv grn') { fT++; fH++; } else if (h.fir === 'miss') { fT++; } if(h.gir === 'hit' || h.gir === 'under') { gT++; gH++; } else if (h.gir === 'miss') { gT++; } });
    ...
    try { if(aCtx && typeof Chart !== 'undefined') { accuracyChart = new Chart(aCtx.getContext('2d'), { type: 'line', data: { labels: accLabels, datasets: [{ label: 'FIR %', data: firData, borderColor: '#8b5cf6', tension: 0.3 }, { label: 'GIR %', data: girData, borderColor: '#d946ef', tension: 0.3 }] }, options: { responsive: true, maintainAspectRatio: false } }); } } catch(e){}
    ...
    try { if(psCtx && typeof Chart !== 'undefined') { parScoringChart = new Chart(psCtx.getContext('2d'), { type: 'bar', data: { labels: ['Par 3', 'Par 4', 'Par 5'], datasets: [{ label: 'Avg Strokes', data: [p3C>0?(p3T/p3C).toFixed(2):0, p4C>0?(p4T/p4C).toFixed(2):0, p5C>0?(p5T/p5C).toFixed(2):0], backgroundColor: ['#f43f5e', '#14b8a6', '#eab308'] }] }, options: { responsive: true, maintainAspectRatio: false } }); } } catch(e){}

    let clubStats = {}; 
    chartData.forEach(r => { (r.hole_scores||[]).forEach(h => { if (h.drive_club && h.drive_distance > 0) { if(!clubStats[h.drive_club]) clubStats[h.drive_club] = {tot:0, cnt:0}; clubStats[h.drive_club].tot += h.drive_distance; clubStats[h.drive_club].cnt++; } if (h.approach_club && h.approach_yd > 0) { if(!clubStats[h.approach_club]) clubStats[h.approach_club] = {tot:0, cnt:0}; clubStats[h.approach_club].tot += h.approach_yd; clubStats[h.approach_club].cnt++; } }); }); 
    practiceSessionData.forEach(s => { if(!clubStats[s.club]) clubStats[s.club] = {tot:0, cnt:0}; clubStats[s.club].tot += s.dist; clubStats[s.club].cnt++; });
    ...
    try { if(cCtx && typeof Chart !== 'undefined') { clubChart = new Chart(cCtx.getContext('2d'), { type: 'bar', data: { labels: cLabels, datasets: [{ data: cData, backgroundColor: '#3b82f6' }] }, options: { responsive: true, maintainAspectRatio: false } }); } } catch(e){}
    ...
    if(mpStat) { mpStat.innerHTML = `<div style="margin-bottom: 10px;"><b>Drive Bias:</b> ${fTotMiss>0 ? `${Math.round((fL/fTotMiss)*100)}% Left | ${Math.round((fR/fTotMiss)*100)}% Right` : 'No data.'}</div><div><b>Approach Bias:</b> ${gTotMiss>0 ? `${Math.round((gS/gTotMiss)*100)}% Short | ${Math.round((gL/gTotMiss)*100)}% Left` : 'No data.'}</div>`; }
};

window.openStatGraph = function(title, statKey) {
    document.getElementById('stat-graph-title').innerText = title.toUpperCase(); document.getElementById('stat-graph-modal').style.display = 'flex'; currentStatKey = statKey; currentStatTitle = title;
    let tfEl = document.getElementById('filter-timeframe'); if (tfEl) { let modTfEl = document.getElementById('modal-filter-timeframe'); if (modTfEl) modTfEl.value = tfEl.value; }
    window.refreshModalGraph();
};

window.refreshModalGraph = function() {
    if(!currentStatKey) return;
    let modTfEl = document.getElementById('modal-filter-timeframe'); let tFilter = modTfEl ? modTfEl.value : 'season';
    let plotData = []; let rList = [...currentFilteredRounds];
    if(tFilter === 'last10') rList = rList.slice(0, 10); if(tFilter === 'last20') rList = rList.slice(0, 20); rList = rList.reverse();
    
    rList.forEach(r => {
        let val = 0; let valid = false; let targetHoles = r.hole_scores || [];
        if(currentStatKey === 'score') { let sSum = 0; targetHoles.forEach(h => { sSum += (h.score||0); }); val = sSum; valid=true; }
        if(currentStatKey === 'putts') { let th=0, tp=0; targetHoles.forEach(h=>{if(h.putts!==null && h.putts>0){tp+=h.putts; th++;}}); if(th>0) { val=(tp/th)*18; valid=true;} }
        if(currentStatKey === 'fir') { let th=0, tf=0; targetHoles.forEach(h=>{if(h.fir==='hit'||h.fir==='miss'||h.fir==='drv grn'){tf++; if(h.fir==='hit'||h.fir==='drv grn')th++;}}); if(tf>0) { val=(th/tf)*100; valid=true;} }
        if(currentStatKey === 'gir') { let th=0, tg=0; targetHoles.forEach(h=>{if(h.gir==='hit'||h.gir==='miss'||h.gir==='under'){tg++; if(h.gir==='hit'||h.gir==='under')th++;}}); if(tg>0) { val=(th/tg)*100; valid=true;} }
        if(currentStatKey === 'drops') { let dSum = 0; targetHoles.forEach(h => { dSum += (h.drops||0); }); val = dSum; valid=true; }
        ...
        if(currentStatKey === 'driveDist') { let drvTot=0, drvCnt=0; targetHoles.forEach(h => { if(h.drive_distance > 0 && (!h.drive_exception || h.drive_exception === "")) { drvTot += h.drive_distance; drvCnt++; } }); if(drvCnt>0) { val = Math.round(drvTot/drvCnt); valid = true; } }
        if(['hio','egl','brd','par','bog','dbl','tpl','qd'].includes(currentStatKey)) {
            let cnt=0; targetHoles.forEach(h => { if(h.score && h.par) { let d = h.score - h.par; if(currentStatKey==='hio' && h.score===1) cnt++; else if(currentStatKey==='egl' && d===-2) cnt++; else if(currentStatKey==='brd' && d===-1) cnt++; else if(currentStatKey==='par' && d===0) cnt++; else if(currentStatKey==='bog' && d===1) cnt++; else if(currentStatKey==='dbl' && d===2) cnt++; else if(currentStatKey==='tpl' && d===3) cnt++; else if(currentStatKey==='qd' && d>=4) cnt++; } }); val = cnt; valid = true;
        }
        if(['p3','p4','p5'].includes(currentStatKey)) { let pt=0, pc=0; targetHoles.forEach(h => { if(h.score && h.par) { let p = parseInt(currentStatKey.replace('p','')); if(h.par===p) { pt+=h.score; pc++; } } }); if(pc>0) { val = pt/pc; valid=true; } }
    ...
    statDetailChartObj = new Chart(cEl.getContext('2d'), { 
        type: 'line', data: { labels: plotData.map(d=>d.x), datasets: [{ label: currentStatTitle, data: plotData.map(d=>d.y), borderColor: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.2)', borderWidth: 2, fill: true, pointBackgroundColor: '#121212', tension: 0.3 }] }, 
...
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', window.initializeApp); } else { window.initializeApp(); }
