...
async function handleForgotPassword() {
    ...
    try {
        ...
        if (error) throw error; document.getElementById('auth-error').style.color = "var(--accent-green)"; document.getElementById('auth-error').innerText = "Recovery email routed.";
...
window.savePracticeSession = function() {
    ...
    alert("Practice array structural arrays committed to memory stack.");
...
// ----------------------------------------------------
window.openPuttMapper = function() { pinPos = null; ballPos = null; document.getElementById('putt-distance-display').innerText = "-- ft"; document.getElementById('putt-map-modal').style.display = 'flex'; clearPuttCanvas(); };
...
window.adjustDrop = function(amount) {
    ...
    if(gridCell) { gridCell.innerText = next === 0 ? '-' : next; gridCell.style.color = next > 0 ? '#ef4444' : 'var(--text-muted)'; }
...
window.toggleDropType = function(type) {
    ...
    ['WATER', 'OB', 'LOST', 'UNPLAYABLE'].forEach(id => { const btn = document.getElementById(`drop-${id.toLowerCase()}`); if(btn) btn.classList.toggle('active', adv.includes(id)); });
    saveLocalState();
};
...
window.cycleSand = function() {
    ...
    let next = (s === "") ? "0" : (s === "0" ? "1" : (s === "1" ? "2" : (s === "2" ? "3+" : "")));
...
// Scorecard Grid to Play Mode data cascade callback hook
...
function updatePlayModeUI() {
    ...
    ['fir', 'gir'].forEach(type => {
        let hb = document.getElementById(`${type}-hit-btn`); let mb = document.getElementById(`${type}-miss-btn`);
        ...
        if(subMenu) {
            ...
            document.querySelectorAll(`#${type}-sub-menu .sub-miss`).forEach(btn => { let val = btn.id.split('-').pop().toUpperCase(); btn.classList.toggle('active', advArr.includes(val)); });
        }
    });
    
    let dBlock = document.getElementById('play-fir-block'); 
    if(dBlock) { document.querySelectorAll('#play-fir-block button, #play-fir-block input, #play-fir-block select').forEach(el => el.disabled = (par == 3)); dBlock.style.opacity = (par == 3) ? '0.3' : '1'; }
}

window.checkHarvesterStatus = function() {
    document.getElementById('settings-overlay').style.display = 'none';
    document.getElementById('insight-detail-title').innerText = "HARVESTER LAYER LOG";
    document.getElementById('insight-detail-content').innerHTML = `<b>Unique Scraped Array Profiles:</b> Active matching processes operating normally across Western Canadian spatial tables.`;
...
function renderCharts(filteredRounds, actHoles, actPars) {
    const tCtx = document.getElementById('scoringTrendChart'); const cCtx = document.getElementById('clubChart'); const wCtx = document.getElementById('weatherScoreChart'); const pC = document.getElementById('scoringPieChart');
    ...
    if (activeOverlay === 'hcp') {
    ...
    } else if (activeOverlay !== 'none' && activeOverlay !== 'driveDist') {
        let oData = [];
        chartData.forEach(r => {
            let p=0, fH=0, fT=0, gH=0, gT=0, dr=0; let th = r.hole_scores || [];
            th.forEach(h => { if(h.putts) p+=h.putts; if(h.drops) dr+=h.drops; if(h.fir==='hit') fH++; if(h.fir==='hit'||h.fir==='miss') fT++; if(h.gir==='hit') gH++; if(h.gir==='hit'||h.gir==='miss') gT++; });
        ...
        trendDatasets.push({ label: activeOverlay.toUpperCase(), data: oData, borderColor: '#38bdf8', backgroundColor: 'transparent', borderWidth: 2, yAxisID: 'y1', tension: 0.3 });
    } else if (activeOverlay === 'driveDist') {
        let oData = [];
        chartData.forEach(r => { let dTot=0, dCnt=0; (r.hole_scores||[]).forEach(h => { if(h.drive_distance>0) { dTot+=h.drive_distance; dCnt++; } }); oData.push(dCnt>0?Math.round(dTot/dCnt):null); });
        trendDatasets.push({ label: 'Avg Drive Distance', data: oData, borderColor: '#8b5cf6', backgroundColor: 'transparent', borderWidth: 2, yAxisID: 'y1', tension: 0.3 });
    }
    ...
    let clubStats = {};
    chartData.forEach(r => { (r.hole_scores||[]).forEach(h => { if (h.drive_club && h.drive_distance > 0) { if(!clubStats[h.drive_club]) clubStats[h.drive_club] = {tot:0, cnt:0}; clubStats[h.drive_club].tot += h.drive_distance; clubStats[h.drive_club].cnt++; } }); });
    let cLabels = Object.keys(clubStats); let cData = cLabels.map(c => Math.round(clubStats[c].tot/clubStats[c].cnt));
    ...
    // INSIGHT ARRAYS TEXT REFLECTION DATA BLOCK INJECTION
    let fL=0, fR=0, fS=0, fTotMiss=0; let gL=0, gR=0, gS=0, gLg=0, gTotMiss=0; let dW=0, dOB=0, dL=0, dU=0, dTot=0;
    chartData.forEach(r => {
    ...
    if(mpStat) {
        mpStat.innerHTML = `
            ...
            <div style="margin-bottom: 10px;"><b>Approach Bias:</b> ${gTotMiss>0 ? `${Math.round((gS/gTotMiss)*100)}% Short | ${Math.round((gL/gTotMiss)*100)}% Left` : 'No data.'}</div>
            <div><b>Hazards Tripped:</b> ${dTot>0 ? `${dW} Water | ${dOB} OB | ${dL} Lost` : '0 Penalties logged.'}</div>
        `;
    }
}
...
window.renderHistoryList = function(allData, type) {
    ...
    let html = "";
    data.forEach(r => {
        const hp = r.hole_scores ? r.hole_scores.filter(h => h.score > 0).length : 0;
        html += `<div class="history-item" onclick="openHistoryModal('${r.id}', '${r.course_name.replace(/'/g, "\\'")}', '${r.date_played}', ${r.total_score}, ${hp}, '${r.weather_temp||''}', '${r.weather_wind||''}')">
...
window.toggleGroupToggles = function(mainCb, childClass, btnTextId, defaultText) { document.querySelectorAll(childClass).forEach(b => b.checked = mainCb.checked); updateAnalytics(); }
window.checkGroupToggles = function(childClass, mainId, btnTextId, defaultText) { const cbs = Array.from(document.querySelectorAll(childClass)); const allChecked = cbs.every(b => b.checked); document.getElementById(mainId).checked = allChecked; updateAnalytics(); }
