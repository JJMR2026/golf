...
window.populateClubDropdowns = function() {
    ...
    if (driveSelect && appSelect) {
        ...
        // Restore values if they exist in the new filtered bag
...
window.updatePlayModeUI = function() {
    ...
    if(!navTop) {
        navTop = document.createElement('div'); navTop.id = 'play-nav-top'; navTop.innerHTML = navHtml;
        statsHeader.after(navTop);
    }
    if(!navBot) {
        navBot = document.createElement('div'); navBot.id = 'play-nav-bot'; navBot.innerHTML = navHtml;
        container.appendChild(navBot);
    }

    // Default Club Assignments
    if (state.score === "") {
        let bag = window.getMyBag();
        if (par == 4 || par == 5) { 
            if (state.driveClub === "") { 
...
window.refreshModalGraph = function() {
    ...
    rList.forEach(r => {
        ...
        if(currentStatKey === 'fir') { 
            let th=0, tf=0; targetHoles.forEach(h=>{if(h.fir==='hit'||h.fir==='miss'||h.fir==='drv grn'){tf++; if(h.fir==='hit'||h.fir==='drv grn')th++;}}); if(tf>0) { val=(th/tf)*100; valid=true;} 
        }
        if(currentStatKey === 'gir') { 
            let th=0, tg=0; targetHoles.forEach(h=>{if(h.gir==='hit'||h.gir==='miss'||h.gir==='under'){tg++; if(h.gir==='hit'||h.gir==='under')th++;}}); if(tg>0) { val=(th/tg)*100; valid=true;} 
        }
